...
<html lang="{{ app()->getLocale() }}">
    <head>
        ...
        <style id="custom_css">
            ...
            {!! \App\Models\Setting::fetch('custom_css') !!}
        </style>
    </head>
    <body>
        <div id="app"{!! $alt_bg !!}>
                ...
                @if(isset($all_apps))
                    ...
                    <ul id="pinlist">
                        @foreach($all_apps as $app)
                                ...
                                ?>
                            <li>{{ $app->title }}<a class="{{ $active }}" data-tag="{{ $tag ?? 0 }}"
                                                    data-id="{{ $app->id }}"
                                                    href="{{ route('items.pintoggle', [$app->id]) }}"><i
                                        class="fas fa-thumbtack"></i></a></li>
                ...
                <main id="main">
                    ...
                    <div id="config-buttons">
                        ...
                        @if(Route::is('dash') || Route::is('tags.show'))
                            <a id="config-button" class="config" href=""><i class="fas fa-exchange"></i>
                                <div class="tooltip left">{{ __('app.dashboard.reorder') }}</div>
                            </a>

                        @endif

                        <a id="dash" class="config" href="{{ route('dash', []) }}"><i class="fas fa-th"></i>
                            <div class="tooltip left">{{ __('app.dashboard') }}</div>
                        </a>
                        @if($current_user->id === 1)
                            <a id="users" class="config" href="{{ route('users.index', []) }}"><i
                                    class="fas fa-user"></i>
                                <div class="tooltip left">{{ __('app.user.user_list') }}</div>
                            </a>
                        @endif
                        <a id="items" class="config" href="{{ route('items.index', []) }}"><i class="fas fa-list"></i>
                            <div class="tooltip left">{{ __('app.apps.app_list') }}</div>
                        </a>
                        <a id="folder" class="config" href="{{ route('tags.index', []) }}"><i class="fas fa-tag"></i>
                            <div class="tooltip left">{{ __('app.apps.tag_list') }}</div>
                        </a>
                        <a id="settings" class="config" href="{{ route('settings.index', []) }}"><i
                                class="fas fa-cogs"></i>
                            <div class="tooltip left">{{ __('app.dashboard.settings') }}</div>
                        </a>
                    </div>
                </main>
        ...
        <script id="custom_js">
            ...
            {!! \App\Models\Setting::fetch('custom_js') !!}
        </script>
    </body>
</html>
